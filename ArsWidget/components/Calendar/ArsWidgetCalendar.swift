...
struct CalendarView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if showComposer {
                HStack(spacing: 8) {
                    TextField("Новое событие…", text: $draftEventTitle)
                    ...
                    .disabled(draftEventTitle.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
                    .opacity(draftEventTitle.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? 0.5 : 1)
                }
                .padding(.top, 8)
            }
    ...
    private func createEvent() {
        let title = draftEventTitle.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !title.isEmpty else { return }
        Task {
            let success = await calendarManager.createEvent(title: title, date: selectedDate)
