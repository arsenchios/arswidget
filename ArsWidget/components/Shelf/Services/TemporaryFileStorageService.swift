...
class TemporaryFileStorageService {
    ...
    func createTempFile(for type: TempFileType) async -> URL? {
        createTempFileSynchronously(for: type)
    }
    ...
    private func createTempFileSynchronously(for type: TempFileType) -> URL? {
        let tempDir = URL(fileURLWithPath: NSTemporaryDirectory())
        let uuid = UUID().uuidString

        switch type {
        case .data(let data, let suggestedName):
            let filename = safeFileName(suggestedName, fallback: "attachment.dat")
