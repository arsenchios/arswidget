        ...
        return await withCheckedContinuation { (cont: CheckedContinuation<Data?, Never>) in
            loadItem(forTypeIdentifier: UTType.data.identifier, options: nil) { item, error in
                ...
                if let url = item as? URL, url.isFileURL, let data = try? Data(contentsOf: url) {
                    // NSItemProvider owns temporary promise files. Never delete or modify a URL
                    // supplied by another app: it may point at a real user-selected file.
    ...
    func loadFileURL(typeIdentifier: String) async -> URL? {
        await withCheckedContinuation { (cont: CheckedContinuation<URL?, Never>) in
            self.loadItem(forTypeIdentifier: typeIdentifier, options: nil) { item, error in
                ...
                cont.resume(returning: resolvedURL?.isFileURL == true ? resolvedURL : nil)
            }
        }
    }
        ...
        await withCheckedContinuation { (cont: CheckedContinuation<URL?, Never>) in
            self.loadItem(forTypeIdentifier: typeIdentifier, options: nil) { item, error in
                ...
                if let url = item as? URL, ["https", "http"].contains(url.scheme?.lowercased() ?? "") {
                    ...
                    if let string = String(data: data, encoding: .utf8) {
                        if let url = URL(string: string), ["https", "http"].contains(url.scheme?.lowercased() ?? "") {
                            cont.resume(returning: url)
                            return
                        }
                    }
