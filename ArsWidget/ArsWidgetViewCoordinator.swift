//
//  ArsWidgetViewCoordinator.swift
//  ArsWidget
//
...
class ArsWidgetViewCoordinator: ObservableObject {
    ...
    @AppStorage("arsWidgetFirstRunTourCompleted") private var firstRunTourCompleted: Bool = false
    ...
    private init() {
        ...
        Task { @MainActor in
            // ArsWidget uses its own in-notch first-run tour instead of the old welcome animation.
    ...
    @discardableResult
    func presentFirstRunTourIfNeeded() -> Bool {
        guard !firstRunTourCompleted else { return false }
        firstLaunch = false
    ...
    func finishFirstRunTour() {
        firstRunTourCompleted = true
        isFirstRunTourPresented = false
        currentView = .home
    }
