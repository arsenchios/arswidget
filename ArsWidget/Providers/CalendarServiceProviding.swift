...
protocol CalendarServiceProviding {
    ...
    func reminders(from start: Date, to end: Date, calendarIDs: [String]) async -> [EventModel]
    func reminders(calendarIDs: [String], includeCompleted: Bool) async -> [EventModel]
    func createReminder(title: String, dueDate: Date?, calendarID: String?) async -> Bool
...
class CalendarService: CalendarServiceProviding {
    ...
    func reminders(from start: Date, to end: Date, calendarIDs ids: [String]) async -> [EventModel] {
        guard hasAccess(to: .reminder) else { return [] }

        let reminderCalendars = store.calendars(for: .reminder).filter { ids.isEmpty || ids.contains($0.calendarIdentifier) }
        return await fetchReminders(from: start, to: end, calendars: reminderCalendars)
            .sorted { $0.start < $1.start }
    }

    func reminders(calendarIDs ids: [String], includeCompleted: Bool = false) async -> [EventModel] {
        guard hasAccess(to: .reminder) else { return [] }

        let reminderCalendars = store.calendars(for: .reminder).filter { ids.isEmpty || ids.contains($0.calendarIdentifier) }
        return await withCheckedContinuation { continuation in
            let predicate = store.predicateForReminders(in: reminderCalendars)
            ...
            store.fetchReminders(matching: predicate) { reminders in
                let models = (reminders ?? [])
                    ...
                    .sorted { lhs, rhs in
                        switch (lhs.start == .distantFuture, rhs.start == .distantFuture) {
                        case (false, true):
                            return true
                        case (true, false):
                            return false
    ...
    // Added in personal fork: create a new reminder from the notch widget.
    func createReminder(title: String, dueDate: Date?, calendarID: String?) async -> Bool {
        guard hasAccess(to: .reminder) else { return false }
        ...
        if let calendarID, let list = store.calendar(withIdentifier: calendarID) {
            reminder.calendar = list
        } else {
            reminder.calendar = store.defaultCalendarForNewReminders()
        }

        guard reminder.calendar != nil else { return false }
    ...
    func createEvent(title: String, startDate: Date, endDate: Date, calendarID: String?) async -> Bool {
        guard hasAccess(to: .event) else { return false }
...
extension EventModel {
    ...
    init?(from reminder: EKReminder) {
        ...
        let date = dueDateComponents.flatMap { Calendar.current.date(from: $0) } ?? .distantFuture
