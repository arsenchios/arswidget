...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var viewModels: [String: ArsWidgetViewModel] = [:] // UUID -> ArsWidgetViewModel
    var window: NSWindow?
    let vm: ArsWidgetViewModel = .init()
    @ObservedObject var coordinator = ArsWidgetViewCoordinator.shared
    var quickShareService = QuickShareService.shared
    var clipboardManager = ClipboardManager.shared // added in personal fork: starts clipboard history tracking at launch
    var vocabManager = VocabManager.shared // added in personal fork: starts the Ukrainian-words timer at launch
    var systemStatsManager = SystemStatsManager.shared // added in personal fork: starts CPU/RAM polling at launch
    ...
    func onScreenLocked(_ notification: Notification) {
        isScreenLocked = true
        PomodoroManager.shared.handleScreenLockChange(isLocked: true)
        VocabManager.shared.dismissPrompt()
    ...
    func onScreenUnlocked(_ notification: Notification) {
        ...
        PomodoroManager.shared.handleScreenLockChange(isLocked: false)
        if !Defaults[.showOnLockScreen] {
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? ArsWidgetSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? ArsWidgetSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func setupDragDetectorForScreen(_ screen: NSScreen) {
        ...
        let notchWidth = maxOpenNotchWidth
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !Defaults[.showOnAllDisplays] {
            ...
            let window = createArsWidgetWindow(
        ...
        // The first-run tour lives inside the widget and requests no unrelated permissions.
        if coordinator.presentFirstRunTourIfNeeded() {
            DispatchQueue.main.async {
                self.vm.open()
            }
        }
    ...
    @objc func adjustWindowPosition(changeAlpha: Bool = false) {
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    let viewModel = ArsWidgetViewModel(screenUUID: uuid)
                    let window = createArsWidgetWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createArsWidgetWindow(for: selectedScreen, with: vm)
            }
